<template>
    <div class="min-h-[calc(100vh-4rem)]">
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500">
            ...
            <div class="absolute bottom-0 left-0 right-0">
                <svg viewBox="0 0 1440 80" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-full">
                    <path d="M0 80L60 74.7C120 69 240 59 360 53.3C480 48 600 48 720 53.3C840 59 960 69 1080 69.3C1200 69 1320 59 1380 53.3L1440 48V80H1380C1320 80 1200 80 1080 80C960 80 840 80 720 80C600 80 480 80 360 80C240 80 120 80 60 80H0Z" class="fill-gray-50 dark:fill-gray-900"/>
                </svg>
            </div>
        </div>
        ...
        <div class="max-w-4xl mx-auto px-4 py-12">
            <!-- Category Filter -->
            <div v-if="categories.length > 0" class="mb-8 flex flex-wrap gap-2">
                <button
                    @click="filterByCategory('')"
                    :class="activeCategory === '' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-600'"
                ...
                <button
                    ...
                    :class="activeCategory === cat.slug ? 'bg-indigo-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-600'"
                    class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-200"
                >
                    {{ cat.name }}
                </button>
            ...
            <div v-else>
                ...
                <div v-if="articles.length === 0" class="text-center py-16">
                    ...
                    <p class="text-gray-400 dark:text-gray-500 text-lg">暂无文章</p>
                </div>
...
async function fetchArticles(page, pp) {
    loading.value = true;
    try {
        const response = await getArticles(page, pp || perPage.value, activeCategory.value);
