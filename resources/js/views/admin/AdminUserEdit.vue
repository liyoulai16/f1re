<template>
    <div>
        ...
        <div v-else class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 max-w-2xl">
            <form @submit.prevent="handleSubmit" class="space-y-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition"
                    />
                    <p class="text-xs text-gray-400 mt-1">留空表示不修改密码</p>
                </div>
...
onMounted(async () => {
    try {
        const response = await getAdminUser(route.params.id);
...
async function handleSubmit() {
    ...
    try {
        ...
        await updateAdminUser(route.params.id, data);
